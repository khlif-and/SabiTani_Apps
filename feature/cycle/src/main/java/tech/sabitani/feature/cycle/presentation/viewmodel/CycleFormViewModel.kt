...
@HiltViewModel
internal class CycleFormViewModel
    @Inject
    constructor(
        ...
        ContainerHost<CycleFormState, CycleFormEffect> {
        private val route = savedStateHandle.toRoute<CycleFormRoute>()

        override val container =
            container<CycleFormState, CycleFormEffect>(
                CycleFormState(
                    plotId = route.plotId,
        ...
        private fun reduce(block: (CycleFormState) -> CycleFormState) =
            intent {
                reduce { block(state) }
            }
