...
internal class CycleDetailViewModel @Inject constructor(
...
) : ViewModel(), ContainerHost<CycleDetailState, CycleDetailEffect> {

    private val route = savedStateHandle.toRoute<CycleDetailRoute>()

    override val container = container<CycleDetailState, CycleDetailEffect>(
    ...
    private fun reduceState(block: (CycleDetailState) -> CycleDetailState) = intent {
        reduce { block(state) }
    }

    private fun openActivityDialog() = reduceState {
        it.copy(activityDraft = ActivityDraft(performedOn = today()))
    }

    private fun openTransactionDialog() = reduceState {
        it.copy(transactionDraft = TransactionDraft(occurredOn = today()))
    }

    private fun updateActivity(block: (ActivityDraft) -> ActivityDraft) = intent {
        state.activityDraft?.let { current -> reduce { state.copy(activityDraft = block(current)) } }
    }

    private fun updateTransaction(block: (TransactionDraft) -> TransactionDraft) = intent {
        state.transactionDraft?.let { current ->
            reduce { state.copy(transactionDraft = block(current)) }
        }
    }

    private fun submitActivity() = intent {
    ...
    private fun submitTransaction() = intent {
        val draft = state.transactionDraft ?: return@intent
        val occurredOn = draft.occurredOn
        val amount = draft.amountText.replace(".", "").replace(",", "").toLongOrNull()
        if (occurredOn == null) {
