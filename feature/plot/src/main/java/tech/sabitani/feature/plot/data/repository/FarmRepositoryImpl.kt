...
internal class FarmRepositoryImpl
    @Inject
    constructor(
        private val farmDao: FarmDao,
        private val clock: Clock,
    ) : FarmRepository {
        override fun observeFarms(): Flow<List<Farm>> = farmDao.observeAll().map { list -> list.map { it.toDomain() } }

        override fun observeFarm(id: Long): Flow<Farm?> = farmDao.observeById(id).map { it?.toDomain() }
