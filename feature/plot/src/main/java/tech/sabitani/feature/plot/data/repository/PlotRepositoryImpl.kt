...
internal class PlotRepositoryImpl
    @Inject
    constructor(
        private val plotDao: PlotDao,
        private val clock: Clock,
    ) : PlotRepository {
        override fun observePlots(farmId: Long): Flow<List<Plot>> = plotDao.observeByFarm(farmId).map { list -> list.map { it.toDomain() } }

        override fun observePlot(id: Long): Flow<Plot?> = plotDao.observeById(id).map { it?.toDomain() }
