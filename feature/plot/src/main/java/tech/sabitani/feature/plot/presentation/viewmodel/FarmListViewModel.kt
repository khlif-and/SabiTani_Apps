...
@HiltViewModel
internal class FarmListViewModel @Inject constructor(
    private val observeFarmsUseCase: ObserveFarmsUseCase,
    private val addFarmUseCase: AddFarmUseCase,
) : ViewModel(), ContainerHost<FarmListState, FarmListEffect> {

    override val container = container<FarmListState, FarmListEffect>(FarmListState()) {
        observeFarmsUseCase()
    ...
    private fun updateName(value: String) = intent {
        reduce { state.copy(draftName = value) }
    }

    private fun updateLocation(value: String) = intent {
        reduce { state.copy(draftLocation = value) }
    }

    private fun updateArea(value: String) = intent {
    ...
    private fun submit() = intent {
        ...
        val areaValue = state.draftTotalAreaText.trim().replace(',', '.')
            .takeIf(String::isNotEmpty)?.toDoubleOrNull()
        val result = addFarmUseCase(
    ...
    private fun navigateToPlots(farmId: Long) = intent {
